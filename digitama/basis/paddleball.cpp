...
void JrLab::PaddleBallWorld::load(float width, float height) {
    TheBigBang::load(width, height);

    this->ball = this->spawn<Circlet>(ball_radius, CHOCOLATE);
    this->paddle = this->spawn<Rectanglet>(paddle_width, paddle_height, ROYALBLUE);
...
void JrLab::PaddleBallWorld::on_mission_start(float width, float height) {
    // 确保球产生于屏幕上方的中间
...
void JrLab::PaddleBallWorld::update(uint64_t count, uint32_t interval, uint64_t uptime) {
...
void JrLab::PaddleBallWorld::on_char(char key, uint16_t modifiers, uint8_t repeats, bool pressed) {
    if (key == 'a') {
        if (pressed) {
