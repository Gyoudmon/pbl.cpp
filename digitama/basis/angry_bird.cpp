...
void JrLab::AngryBirdWorld::load(float width, float height) {
    TheBigBang::load(width, height);

    this->set_background(LIGHTBLUE);

    this->ground = this->spawn<MarioGroundAtlas>(0U, 3, 75);
    this->catapult = this->spawn<Sprite>(digimon_path("assets/catapult", ".png"));

    this->angry_bird = this->spawn<SpriteGridSheet>(digimon_path("assets/AngryBirds", ".png"), 1, 5);
    this->king_pig = this->spawn<SpriteGridSheet>(digimon_path("assets/Pigs", ".png"), 1, 9);
...
void JrLab::AngryBirdWorld::reflow(float width, float height) {
...
void JrLab::AngryBirdWorld::update(uint64_t count, uint32_t interval, uint64_t uptime) {
}

// 实现 AngryBirdWorld::on_char 方法，处理键盘事件，用于控制桨的移动
void JrLab::AngryBirdWorld::on_char(char key, uint16_t modifiers, uint8_t repeats, bool pressed) {
}
