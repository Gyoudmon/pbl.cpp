...
/*************************************************************************************************/
void JrLab::ColorMixtureWorld::load(float width, float height) {
    TheBigBang::load(width, height);

    /* 思考：为什么背景一定要是黑色的？ */
    this->set_background(0x000000U);

    this->half_white = this->spawn<Rectanglet>(width * 0.5F, height, 0xFFFFFFU);
    this->half_white->camouflage(true);

    this->red = this->spawn<Circlet>(radius, 0xFF0000U);   // 红色
    this->green = this->spawn<Circlet>(radius, 0x00FF00U); // 绿色
    this->blue = this->spawn<Circlet>(radius, 0x0000FFU);  // 蓝色
    ...
    this->cyan = this->spawn<Circlet>(radius, 0x00FFFFU);    // 青色
    this->magenta = this->spawn<Circlet>(radius, 0xFF00FFU); // 品红
    this->yellow = this->spawn<Circlet>(radius, 0xFFFF00U);  // 黄色
...
void JrLab::ColorMixtureWorld::reflow(float width, float height) {
...
void JrLab::ColorMixtureWorld::after_select(IMatter* m, bool yes) {
    if (!yes) {
